...
public class PostResolvingHelpService implements PostResolvingHelpUsecase {
    ...
    public List<ResolvedPost> resolvePostsByIds(List<Long> postIds) {
        ...
        List<ResolvedPost> resolvedPostList = new ArrayList<>();
        resolvedPostList.addAll(resolvedPostCachePort.multiGet(postIds));
        List<Long> missingPostIds = postIds.stream()
                .filter(postId -> resolvedPostList.stream().noneMatch(resolvedPost -> resolvedPost.getId().equals(postId)))
                .toList();
        ...
        List<ResolvedPost> missingResolvedPosts = missingPosts.stream()
                ...
                .toList();
        resolvedPostList.addAll(missingResolvedPosts);

        Map<Long, ResolvedPost> resolvedPostMap = resolvedPostList.stream().
    ...
    @Override
    public void deleteResovedPost(Long postId) {
